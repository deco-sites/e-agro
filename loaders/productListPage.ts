...
export default async function loader(
    ...
    const { products } = getFilteredProducts(productsData, filtersParamsRecord, count)

    return products
}

const getFilteredProducts = (products: Product[], filtersParams: Record<string, string>, count: number) => {
    const filteredProducts = products.filter(product => {
        return Object.entries(filtersParams).every(([key, value]) => {
            return product.additionalProperty?.some(property => property.name === key && property.value === value)
        })
    })

    const properties = filteredProducts.flatMap(product => product.additionalProperty).filter(property => !!property)
